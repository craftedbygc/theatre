...
- Known pre-existing failure (not caused by env setup): `yarn test` fails only on `devEnv/verify-docker-compose.test.ts` because `packages/remote` isn't listed in `docker-compose.yml`. All other suites pass. Don't chase this while doing unrelated work.
